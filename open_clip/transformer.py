    ...
    def forward(
        ...
        if isinstance(self.attn, Attention):
            if isinstance(q_x, list):
                x, x_ori = q_x
                x_res = self.ls_1(self.attention(self.ln_1(x_ori)))
                x_res, x_ori_res = x_res
                if count % (all / 4) == 0 and count != 0:
                    x = x_res
                else:
            ...
            # start of dual path
                ...
                x_res = self.ls_1(self.attention(self.ln_1(q_x)))
...
class Transformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, attn_mask: Optional[torch.Tensor] = None, layers_ls: List[int] = None,
                vision: bool = False):
        out = []
        for count, r in enumerate(self.resblocks):
            if self.grad_checkpointing and not torch.jit.is_scripting():
                # TODO: handle kwargs https://github.com/pytorch/pytorch/issues/79887#issuecomment-1161758372
                x = checkpoint(r, x, None, None, attn_mask)
            else:
                x = r(x, count, len(self.resblocks), attn_mask=attn_mask)
                if vision and (count + 1) in layers_ls:
                    out.append(x)
        if vision:
            return out
        else:
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, layers_ls: List[int]):

        if self.attn == None:

            # apply architecture surgery on the last 6 blocks
            for i in range(1, len(self.transformer.resblocks) + 1):  # surgery 7, maskclip 2  -->  1, 2, 3, 4, 5, 6
        ...
        results_ls = self.transformer(x, layers_ls=layers_ls, vision=True)
        out = {'ori': [], 'surgery': []}
        for result in results_ls:
            x, x_ori = result
            x[0, :, :] = x_ori[0, :, :]  # clip_surgery  替换掉cls token
            x = x.permute(1, 0, 2)  # LND -> NLD

            x = self.ln_post(x)
            x = x @ self.proj
            out['surgery'].append(x)

            x_ori = x_ori.permute(1, 0, 2)  # LND -> NLD
            out['ori'].append(x_ori)
