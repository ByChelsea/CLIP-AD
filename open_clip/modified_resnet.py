...
class AttentionPool2d(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.attn == None:
            self.attn = Attention(self.output_dim, self.embed_dim, self.num_heads, True, settings='resnet')
            self.attn.qkv.weight = torch.nn.Parameter(
                torch.cat([self.v_proj.weight, self.v_proj.weight, self.v_proj.weight], 0))
...
class ModifiedResNet(nn.Module):
    ...
    def forward(self, x, layers):
        ...
        for layer in layers:
            x = layer(x)
            B, C, H, W = x.shape
            fea = x.view(B, C, -1).permute(0, 2, 1).contiguous()
            out['ori'].append(fea)
